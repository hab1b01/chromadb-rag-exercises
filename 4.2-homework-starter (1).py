...
print_results("Exercise 2A — high priority + year 2025 + verified", exercise_2a_results)

exercise_2b_results = collection.get(
    where={
        "$or": [
            {"category": {"$eq": "software"}},
            {"category": {"$eq": "printing"}},
        ]
    },
    include=["documents", "metadatas"],
)
print_results("Exercise 2B — software OR printing", exercise_2b_results)
...
print_results("Exercise 3A — contains 'student'", exercise_3a_results)
...
print_results("Exercise 3B — contains 'student' but NOT 'password'", exercise_3b_results)

excluded_count = len(exercise_3a_results["ids"]) - len(exercise_3b_results["ids"])
print(f"\n  Excluded by $not_contains='password': {excluded_count} document(s)")
...
print_results("Exercise 4 — filtered (printing + 'page')", exercise_4_filtered_results, show_distances=True)
...
print_results("Exercise 4 — no category filter (for comparison)", exercise_4_unfiltered_results, show_distances=True)

print("\n  Observation: filtered results have lower (better) distances because")
print("  restricting to 'printing' removes semantically weaker matches from other categories.")
